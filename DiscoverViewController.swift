...
class DiscoverViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, XMLParserDelegate  {
    ...
    func beginParsing()
    {
        ...
        if language == "es" || language == "es-MX" { //ESPAÑOL
            parser = XMLParser(contentsOf:(URL(string:"http://flaresoftware.com/scroller/es/listas.xml"))!)!
        } else {
            parser = XMLParser(contentsOf:(URL(string:"http://flaresoftware.com/scroller/en/setlists.xml"))!)!
        }
        setlistTitlesArray = []
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let language = Locale.current.languageCode
        if language == "es" || language == "es-MX" { //ESPAÑOL
            if let url  = URL(string: "http://flaresoftware.com/scroller/es/setlistImg\((indexPath as NSIndexPath).row).png"),
            ...
            if let url  = URL(string: "http://flaresoftware.com/scroller/en/setlistImg\((indexPath as NSIndexPath).row).png"),
                let data = try? Data(contentsOf: url)
            {
                cell.setlistImage.image = UIImage(data: data)
            }
    ...
    lazy var refreshControl: UIRefreshControl = {
        ...
        refreshControl.addTarget(self, action: #selector(DiscoverViewController.handleRefresh), for: UIControlEvents.valueChanged)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "DshowSetlist" {
            if let indexPath = listTableView.indexPathForSelectedRow {
                let controller = (segue.destination) as! DiscoverSetlistViewController
                var songsStart: Int
                var songsEnd: Int
